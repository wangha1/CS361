	...
	for i in range(len(list1)):
		...
		for x in range(len(words)):
			...
			for y in range(len(ignoreWords)):
				if(stripPunc(temp[0]) == ignoreWords[y]):
					flag = 1;
					break;
			if(flag == 0): 
				list2.append((temp,i));
...
def LP(lineList):
	...
	for i in range(len(words)):
		for p1 in range(len(words[i])):
			for p2 in range(p1 + 1, len(words[i])):
				pairs = [words[i][p1], words[i][p2]]
				pairs.sort()
				wd1 = pairs[0]
				wd2 = pairs[1]
				#list_of_pairs.append(pairs);
				if pairs in list_of_pairs:
					if(pair == {}):
						pair[(wd1,wd2)] = 2
					else:
						#searchResult = binarySearch(no_num_pair,pairs);
						if (wd1,wd2) in pair:
							pair[(wd1,wd2)] += 1
						else:
							pair[(wd1,wd2)] = 2
...
#		pairs = sorted([[words[i][p1], words[i][p2]] for p1 in range(len(words[i])) for p2 in range(p1+1,len(words[i]))]);
#		list_of_pairs = list_of_pairs + pairs;
...
#			pair.append(temp);
	for item in pair:
		temp = (item,pair[item])
		return_pair.append(temp)
		#item = tuple(item);
#return_pair.append(item)
	return_pair.sort()
	return return_pair;
